...
static bool rivian_tx_hook(const CANPacket_t *msg) {
  const TorqueSteeringLimits RIVIAN_STEERING_LIMITS = {
    ...
    // Safety must permit anything the software may send: a flat 440 below 13 m/s covers the 9 m/s anchor
    // (440) and the 13-knee (420), then tracks the highway points exactly -- >= software at every speed.
    // NOTE: panda's lookup_t is fixed at 3 x/y elements -- stay 3-point.
    .max_torque_lookup = {
      {13., 25., 27.},
      {440, 325, 305},
    },
