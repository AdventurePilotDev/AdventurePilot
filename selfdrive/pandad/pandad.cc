...
static std::vector<std::string> ignored_panda_serials() {
  // bridge pandas (e.g. the Rivian long-kit black panda) are USB-present but deliberately
  // unmanaged; pandad.py exports their serials so the new-panda watch doesn't restart-loop on them
  std::vector<std::string> ignored;
  const char *env = getenv("PANDAD_IGNORE_SERIALS");
  if (env != nullptr) {
    std::string s(env);
    size_t start = 0;
    while (start <= s.size()) {
      ...
      if (end > start) ignored.push_back(s.substr(start, end - start));
      if (end == s.size()) break;
      start = end + 1;
    }
  }
...

  {
    ...
    if (!ignition_opt.value()) {
      ...
      if (!comms_healthy) {
        ...
        static const std::vector<std::string> ignored_serials = ignored_panda_serials();
        for (std::string &s : Panda::list(true)) {
          if (std::count(ignored_serials.begin(), ignored_serials.end(), s)) continue;
...
void pandad_main_thread(std::vector<std::string> serials) {
  if (serials.size() == 0) {
    ...
    // never self-select bridge pandas when run without explicit serials
    const std::vector<std::string> ignored = ignored_panda_serials();
    serials.erase(std::remove_if(serials.begin(), serials.end(), [&ignored](const std::string &s) {
      return std::count(ignored.begin(), ignored.end(), s) > 0;
    }), serials.end());
