...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
             ...
             low_speed_override: bool = True, prev_track_id: int | None = None) -> RadarLead:
...
class RadarD:
  ...
  def update(self, sm: messaging.SubMaster, rr: car.RadarData):
    ...
    if len(leads_v3) > 1:
      # Asymmetric filter on lead prob to keep lead when uncertain (from dev base)
      for i in range(2):
      ...
      one = get_lead(self.v_ego, self.ready, self.tracks, leads_v3[0], model_v_ego, self.lead_prob_filters[0].x,
                     self.CP, self.CP_SP, low_speed_override=True, prev_track_id=sticky0)
      two = get_lead(self.v_ego, self.ready, self.tracks, leads_v3[1], model_v_ego, self.lead_prob_filters[1].x,
                     self.CP, self.CP_SP, low_speed_override=False, prev_track_id=sticky1)
